...
map.getView().fit([8020697.062490, 1820319.243836, 17261099.299664, 7048967.501323], map.getSize());
        ...
        if (doPopup) {
            ...
            for (var i=0; i<currentFeatureKeys.length; i++) {
                if (currentFeatureKeys[i] != 'geometry') {
                    popupField = '';
					if(layer.get('fieldAliases')[currentFeatureKeys[i]] == "location") {
		...
		for (var i=0; i<currentFeatureKeys.length; i++) {
            if (currentFeatureKeys[i] != 'geometry') {
				...
				if(field == "location") {
					...
					province = currentFeature.get(currentFeatureKeys[i]);
				} else if(field == "city") {
					city = currentFeature.get(currentFeatureKeys[i]);
				} else if(field == "img") {
					img = currentFeature.get(currentFeatureKeys[i]);
				} else {
					info = currentFeature.get(currentFeatureKeys[i]);
				}
            }
        }
		popupText += "<div id='loc-name' class='loc-name'>" + name + "</div>" +
							...
							"</div>" +
							"<hr>" +
							"<div id='loc-info' class='loc-info'>" + info + "</div>"
